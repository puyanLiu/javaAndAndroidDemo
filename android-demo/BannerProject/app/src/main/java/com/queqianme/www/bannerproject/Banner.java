...
//        Log.i(tag,"currentItem: "+currentItem);
    ...
    public void onPageSelected(int position) {
        ...
        if (bannerStyle == BannerConfig.CIRCLE_INDICATOR ||
            ...
            ImageView img = indicatorImages.get((lastPosition - 1 + count) % count);
            img.getLayoutParams().width = mIndicatorWidth;
            img.getLayoutParams().height = mIndicatorHeight;
            indicatorImages.get((position - 1 + count) % count).setImageResource(mIndicatorSelectedResId);

            ImageView img2 = indicatorImages.get((position - 1 + count) % count);
            img2.getLayoutParams().width = mIndicatorSelectedWidth;
            img2.getLayoutParams().height = mIndicatorHeight;
